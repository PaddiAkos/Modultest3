...
5. feladat (3 pont)
    ...
    SELECT * FROM `country` WHERE `LifeExpectancy` IS NULL AND `Population` >= 1;
...
9. feladat (5 pont)
    ...
    SELECT Code,country.Name, city.Name FROM country LEFT JOIN city ON country.Capital = city.ID ORDER BY country.Name ASC;
...
11. feladat (5 pont)
    ...
    SELECT city.ID,city.Name,city.CountryCode,city.District,city.Population FROM city INNER JOIN country ON city.CountryCode = country.Code WHERE country.Region LIKE '%Nordic Countries%' ORDER BY city.`CountryCode`, city.`Population` DESC;
...
12. feladat (5 pont)
   ...
   SELECT Region,COUNT(Region) AS orszagokSzama FROM country WHERE IndepYear IS NULL OR IndepYear < 1500 GROUP BY Region ORDER BY COUNT(Region) DESC;
...
13. feladat (6 pont)
   ...
   SELECT country.* FROM `country` INNER JOIN `countrylanguage` ON `country`.`Code` = `countrylanguage`.`CountryCode` WHERE `countrylanguage`.`IsOfficial` = 'F' AND `Language` = 'English';
...
14. feladat (6 pont)
    ...
    SELECT * FROM country LEFT JOIN city ON city.CountryCode = country.Code WHERE city.Name IS NULL;
...
15. feladat (6 pont)
   ...
   SELECT `Name`,`Continent`,`Region`,`Language` FROM `country` INNER JOIN `countrylanguage` ON `country`.`Code` = `countrylanguage`.`CountryCode` WHERE `countrylanguage`.`Percentage` = 0.0;
